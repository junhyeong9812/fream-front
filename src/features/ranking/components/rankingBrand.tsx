...
const RankingBrand: React.FC = () => {
    ...
    for (let index = 0; index < rankingList.length + 1; index++) {
      if (rankingList[index] && rankingList[index].imgs) {
        ...
        setNextArrow((prev) => {
          const updated = [...prev];
          updated[index] =
            currentIndices[index] < rankingList[index].imgs.length - 3; // 15까지 보이도록 제한
          return updated;
        });
      }
    }
  ...
  const handleScroll = () => {
    ...
    // 페이지 끝에 도달했는지 확인
    if (
      scrollTop + clientHeight >= scrollHeight - 450 &&
      !loading &&
      !allDataLoaded
    ) {
        ...
        {rankingList.slice(0, rankingListCount).map((ranking, brandIndex) => (
          <div className="ranking_brand_content" key={brandIndex}>
            <div className="ranking_brand_info">
              ...
              <img
                src={ranking.logo}
                className="ranking_brand_info_logo"
                alt=""
              />
              <div className="ranking_brand_info_name">
                <div className="ranking_brand_info_name_eng">
                  {ranking.brandEng}
                  <svg
                    className="ranking_brand_info_name_eng_arrow"
                    viewBox="0 0 24 24"
                    fill="none"
                ...
                <div className="ranking_brand_info_name_kor">
                  {ranking.brandKor} · 관심 {ranking.interest}
                </div>
              </div>
              ...
              {ranking.imgs.map((img, index) => (
                <div>
                  {prevArrow[brandIndex] && (
                    <div
                      onClick={() => handlePrev(brandIndex)}
                      className="ranking_brand_img_left_arrow_content"
                    >
                      <svg
                        viewBox="0 0 24 24"
                        fill="none"
                  ...
                  <div
                    className="ranking_brand_img_wrapper"
                    style={{
                      transform: calculateTransform(brandIndex),
                      transition: "transform 0.3s ease-in-out",
                    }}
                  >
                    <div className="ranking_brand_img_content" key={index}>
                      <PiBookmarkSimpleThin className="ranking_brand_img_bookmark" />
                      <img className="ranking_brand_img" src={img} alt="" />
                    </div>
                  </div>
                  ...
                  {nextArrow[brandIndex] && (
                    <div
                      onClick={() =>
                        handleNext(brandIndex, ranking.imgs.length)
                      }
                      className="ranking_brand_img_right_arrow_content"
                    >
                      <svg
                        viewBox="0 0 24 24"
                        fill="none"
            ...
            <div className="ranking_brand_more_content">브랜드 더보기</div>
          </div>
        ))}
      </div>
